...
(function ($) {
  ...
  function wupFormatPriceHtml(price) {
    ...
    return '<span class="woocommerce-Price-amount amount"><bdi>' + '<span class="woocommerce-Price-currencySymbol">' + symbol + '</span>' + parseFloat(price).toFixed(2) + '</bdi></span>';
  }
...
(function ($) {
  ...
  $(document.body).on('added_to_cart', function (e, fragments, cart_hash, $button) {
    const productId = $button ? $button.data('product_id') || $button.closest('[data-product_id]').data('product_id') : 0;
...
(function ($) {
  ...
  function showNext() {
    ...
    var msg = cfg.msg_template.replace('{{name}}', name).replace('{{city}}', city).replace('{{product}}', product.name).replace('{{time}}', randomTime());
    $popup.find('.wup-sp__message').html(msg);
    $popup.find('.wup-sp__image img').attr({
      src: product.image,
      alt: product.name
    });

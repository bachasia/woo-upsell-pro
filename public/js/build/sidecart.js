...
(function ($) {
  ...
  function cartAction(action, data, callback) {
    $.post(cfg.ajax_url, Object.assign({
      action: action,
      nonce: cfg.nonce
    }, data), function (res) {
      if (res.success) {
  ...
  $(document).on('click', '.wup-apply-coupon', function () {
    ...
    if (!code) return;
    cartAction('wup_sc_apply_coupon', {
      coupon_code: code
    });
  });
  ...
  $(document).on('click', '.wup-remove-coupon', function () {
    cartAction('wup_sc_remove_coupon', {
...
/******/ })()
;
//# sourceMappingURL=sidecart.js.map